...
def extract_yolo_annotations(ann_filepath):
    ...
    for obj in objs:
        ...
        decoded_masks = maskUtils.decode(rle)
        masks, _ = cv2.findContours(decoded_masks, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        ...
        for m in masks:
            yolo_annotations[formatted_timeframe].append(polygon_to_yolo_str(m / [img_width, img_height], consts.KITTI2COCO[class_id]))
        ...
        for ann_filename in sorted(os.listdir(kitti_anno_dir)):
            ...
            if yolo_annotations:
                for yolo_img_timeframe_label, yolo_img_objects in yolo_annotations.items():
                    ...
                    yolo_ann_filepath = os.path.join(output_dir_labels, sequence, yolo_ann_filename)
                    # TODO: check
                ...
                for root, _, files in os.walk(img_dir):
                    for file in files:
                        if file.endswith(".png") or file.endswith(".jpg"):
                            ...
                            dst_file_path = os.path.join(ouput_folder_image_path, file)
