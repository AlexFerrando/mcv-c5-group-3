...
class FoodDataset(Dataset):
    def __init__(
        ...
        # Define df
        if df is None:
            self.df = pd.read_csv(data_path / 'annotation_llm_results_with_generated.csv')
        else:
            self.df = df
        # Keep only the title and image name
        self.df = self.df[['Title', 'Image_Name', 'Classification', 'Annotation']]
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if not row['Image_Name'].endswith("gen.png"):
