...
def run_experiment_num_denoising_steps(pipe: DiffusionPipeline, user: str, device: str):
    ...
    """
    # Read prompts
    prompts = read_prompts(user, 'uri/title.txt')

    # Define output directory
    output_dir = f"outputs/drinks/title"
    os.makedirs(output_dir, exist_ok=True)
...
def run_negative_prompt_experiment(pipe: DiffusionPipeline, user: str, device: str):
    ...
    """
    # Read prompts
    prompts = read_prompts(user, 'simple.txt')
...
def run_cfg_strength_experiment(pipe: DiffusionPipeline, user: str, device: str):
    ...
    for scale in cfg_scales:
        total_time = 0.0
        pipe.scheduler.set_timesteps(30)  # Set the number of denoising steps
...
def extract_title_slug(prompt: str) -> str:
    """
    Extracts the drink title from a prompt like:
    "A realistic and delicious glass of newton's law drink"
    and returns it in the form: "newton's-law-generated"
    
    Assumes consistent structure, removes the last word.
    """
    try:
        # Remove everything before 'glass of'
        after_glass_of = prompt.split("glass of", 1)[1].strip()
        
        # Remove last word (assumed to be 'drink')
        words = after_glass_of.split()
        title_words = words[:-1]  # remove last word
        title = " ".join(title_words)
        
        # Format: replace spaces with hyphens, lower-case
...
def run_model_comparison(pipe: DiffusionPipeline, user: str, device: str):
    ...
    negative_prompt = (
        "cartoon, illustration, drawing, painting, anime, collage, watermark, text, letters, words, "
        "blurry, deformed, disfigured, bad anatomy, extra limbs, extra fingers, mutation, oversaturated, "
        "low quality, low resolution, artificial, computer generated, multiple drinks, extra drinks, "
        "duplicated objects, multiple cups, cluttered background, busy background, fruits, fruit, "
        "objects, messy, background noise, unwanted objects, animal, animals, pet, dog, cat, bird, "
        "insect, creature, wildlife, plant-only, no drink, empty scene, no glass"
    )

    # Define output directory
    output_dir = f"outputs/drinks/final_negative_prompting"
    os.makedirs(output_dir, exist_ok=True)
    ...
    target_height = 169
    scale = 5
    gen_width = (target_width * scale) // 8 * 8 
    gen_height = (target_height * scale) // 8 * 8
    ...
    for i, prompt in enumerate(prompts):
        ...
        # Downscale cleanly to target resolution
        resized_image = image.resize((target_width, target_height), Image.LANCZOS)

        # Get name
        drink_name = extract_title_slug(prompt)
        filename = f"{output_dir}/{drink_name}.png"
    ...
    end_time = time.time()
    elapsed = end_time - start_time
    print(f"Image generation completed at resolution {gen_height}x{gen_width} in {elapsed:.2f} seconds.")
...
def run_experiment_food(pipe: DiffusionPipeline, user: str, device: str):
    ...
    # Define output directory
    output_dir = f"outputs/drinks/title/{model_name}"
    os.makedirs(output_dir, exist_ok=True)
